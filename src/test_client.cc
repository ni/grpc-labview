...
{
    auto target_str = grpc_labview_tests::GetServerAddress(argc, argv);
    auto certificatePath = grpc_labview_tests::GetCertPath(argc, argv);

    std::shared_ptr<grpc::ChannelCredentials> creds;
    if (!certificatePath.empty())
    {
        std::string cacert = grpc_labview_tests::read_keycert(certificatePath);
    ...
    grpc_labview_tests::QueryClient client(channel);
